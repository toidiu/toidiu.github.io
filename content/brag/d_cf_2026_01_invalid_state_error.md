...
- initial changes to add logs resulted in too many logs so we reverted the change and switched to internal debugging.
- shifted to deploying changes on 10 test staging metals to root cause.
- there were valid reasons for the error (receiving invalid packets from the peer)
...
- With reduced noise it now possible to tell if there are real errors due to regression.
